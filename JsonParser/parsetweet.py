...
class TextCNN(object):
    ...
    def __init__(self,  embedding_array):
        ...
        self.user_input = tf.placeholder(tf.float32, [3402,156])
        user_W1 = tf.Variable(tf.truncated_normal([156,100], stddev=0.1), name="W")
        user_b1 = tf.Variable(tf.constant(0.1, shape=[100]), name="b")
        cap_score = tf.nn.xw_plus_b(self.user_input, user_W1, user_b1)
        cap_score = tf.tanh(cap_score)
        user_W2 = tf.Variable(tf.truncated_normal([100, 1], stddev=0.1), name="W")
        user_b2 = tf.Variable(tf.constant(0.1, shape=[3402]), name="b")
        scal_score = tf.sigmoid(tf.nn.xw_plus_b(cap_score,user_W2,user_b2))
